 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "    \n",
    "    # 7. Python/Non-bash penalty\n",
    "    if any(k in cmd for k in [\"import \", \"def \", \"class \", \"print(\", \"return \"]):\n",
    "        reward -= 2.0\n",
    "    \n",
    "    # 6. Brevity penalty\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "    if iteration < 5:\n",
    "        # ONLY allow known easy categories from local tasks + labeled easy ones\n",
    "        filtered_tasks = [t for t in all_tasks if t.get(\"category\", \"\").lower() in [\"easy\", \"basic\", \"files\", \"text\"]]\n",
    "    elif iteration < 15:\n",
    "        filtered_tasks = [t for t in all_tasks if t.get(\"category\", \"\").lower() in [\"easy\", \"basic\", \"files\", \"text\", \"medium\", \"search\", \"script\"]]\n",
    "    else:\n",
    ...
    "    prompts_text = []\n",
    "    sys_prompt = (\n",
    "        \"You are an expert Linux terminal assistant confined to a raw bash environment. \"\n",
    "        \"The current working directory is /app. \"\n",
    "        \"Your goal is to provide a single, executable bash script that solves the user's task. \"\n",
    "        \"First, analyze the task in <thinking> tags to plan your approach. \"\n",
    "        \"Then, output the final solution inside a ```bash``` code block. \"\n",
    "        \"CRITICAL: The content of the code block must be ONLY executable bash commands. Do NOT include comments, explanations, or markdown. Just the code.\"\n",
    "        \"\\n\\nExample:\\n\"\n",
    "        \"User: List all txt files\\n\"\n",
    "        \"Assistant: <thinking>I need to use the find command to locate .txt files in the current directory.</thinking>\\n```bash\\nfind . -name '*.txt'\\n```\"\n",
    "    )\n",
    "    \n",
